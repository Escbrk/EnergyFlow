...
if(r<s-1){c.push(s);//! Last page
}}const g=c.map(p=>`<li class="pages_list-item">
                <button type="button" class="pages_list-btn">${p}</button>
            </li>`).join("");i.pagination.innerHTML=g,[...document.querySelectorAll(".pages_list-btn")].forEach(p=>{p.textContent.trim()==="..."&&p.classList.add("disabled")})},$=e=>{const s=window.innerWidth<=767?8:12,a=JSON.parse(localStorage.getItem("favorites"))||[],o=a.length,t=Math.ceil(o/s),r=(e-1)*s,n=r+s;return w(e,t),a.slice(r,n)},k=e=>{[...document.querySelectorAll(".pages_list-btn")].forEach(a=>{const o=parseFloat(a.textContent),t=parseFloat(e);o===t?(a.classList.add("active"),a.disabled=!0):(a.classList.remove("active"),a.disabled=!1)})},W=async(e,s)=>{i.exerciseSearchForm.classList.add("hidden"),i.exerciseList.innerHTML=`<li>${T()}</li>`;try{const{results:a,totalPages:o,page:t}=await F(e,s),r=a.map(({imgUrl:n,name:c,filter:g})=>`
          <li class="exercise-item" style="--img: url(${n})">
            <h3 class="exercise-subtitle">${b(c)}</h3>
            <p class="exercise-name">${g}</p>
          </li>
          `).join("");if(!a.length)throw new Error;i.exerciseList.innerHTML=r,i.categoryContainer.innerHTML="",document.querySelector(".search-input").value="",w(t,o),k(t)}catch({message:a}){i.exerciseList.innerHTML="",f.error({title:"Error",message:`Failet to fetch exercises ${a}`})}},A=async({category:e,query:s,currentPage:a,searchTarget:o})=>{i.exerciseList.innerHTML=`<li>${T()}</li>`;try{const{results:t,totalPages:r,page:n}=await D(e,s,a,o);let c=t.map(({target:p,rating:S,burnedCalories:m,bodyPart:y,time:d,name:v,_id:P})=>{const B=String(S.toFixed(1));return`
              ...
              <span class="info-ranking">${B}
                ...
                <div class="info-wrapper">
                  ...
                  <ul class="info-list">
                    ...
                    <li class="info-item">Target: <span>${b(p)}</span></li>
                  </ul>
            </li>
    `}).join("");i.categoryContainer.innerHTML=`<span class="exercise-category">${b(l.query)}</span>`;let g;return c.trim()===""?(g=!0,c='<h3 class="unsucces-title">Unfortunately, <span>no results</span> were found. You may want to consider other search options to find the exercise you are looking for. Our range is wide and you have the opportunity to find more options that suit your needs.</h3>'):g=!1,i.exerciseList.innerHTML=c,g?w():w(n,r),k(n),g}catch(t){console.log(t),f.error({title:"Error",message:"Failed to fetch information about exercises"}),i.exerciseList.innerHTML="",i.exerciseSearchForm.style.display="none"}},E=e=>{const s=e.map(({name:a,bodyPart:o,burnedCalories:t,time:r,target:n,_id:c})=>`
    ...
    `).join("");i.favList&&(i.favList.innerHTML=s,i.emptyWrapper.style.display=e.length===0?"flex":"none",k(l.activeLocalPage))};l.currentPage!=="index.html"&&E($(l.activeLocalPage));i.pagination.addEventListener("click",e=>{if(e.target.classList.contains("pages_list-btn")){const s=parseFloat(e.target.textContent);if(isNaN(s))return;const a=document.querySelector(".exercise-info"),o=document.querySelector(".exercises-section"),t=document.querySelector(".fav-wrapper");o&&(o.scrollIntoView(),a?A({category:l.category,query:l.query,currentPage:s,searchTarget:l.searchTarget}):W(l.query,s)),t&&(t.scrollIntoView(),activePage=s,E($(activePage)))}});const R=e=>{const a=JSON.parse(localStorage.getItem("favorites")).filter(({_id:o})=>o!==e);localStorage.setItem("favorites",JSON.stringify(a)),f.success({title:"Succes",message:"Succesfully deleted from your favorite list"})},J=e=>`
  ...
  `;i.backdrop.querySelector(".exercise").innerHTML=v,i.backdrop.classList.remove("hidden"),document.body.classList.add("noScroll")}catch{f.error({title:"Error",message:"Failed to fetch current exercise"}),setTimeout(()=>{i.backdrop.classList.add("hidden")},0)}};export{i as a,A as b,U as c,_ as d,w as e,R as f,l as g,E as h,$ as l,W as r,j as s};
//# sourceMappingURL=modal-CIkBAFzJ.js.map
