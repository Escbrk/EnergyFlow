...
if(r<s-1){c.push(s);//! Last page
            ...
            </li>`).join("");i.pagination.innerHTML=p,[...document.querySelectorAll(".pages_list-btn")].forEach(d=>{d.textContent.trim()==="..."&&d.classList.add("disabled")})},P=e=>{const s=window.innerWidth<=767?8:12,a=JSON.parse(localStorage.getItem("favorites"))||[],n=a.length,t=Math.ceil(n/s),r=(e-1)*s,o=r+s;return $(e,t),a.slice(r,o)},E=e=>{[...document.querySelectorAll(".pages_list-btn")].forEach(a=>{const n=parseFloat(a.textContent),t=parseFloat(e);n===t?(a.classList.add("active"),a.disabled=!0):(a.classList.remove("active"),a.disabled=!1)})},A=async(e,s)=>{i.exerciseSearchForm.classList.add("hidden"),i.exerciseList.innerHTML=`<li>${B()}</li>`;try{const{results:a,totalPages:n,page:t}=await H(e,s),r=a.map(({imgUrl:o,name:c,filter:p})=>`
          <li class="exercise-item" style="--img: url(${o})">
            <h3 class="exercise-subtitle">${b(c)}</h3>
            <p class="exercise-name">${p}</p>
          </li>
          `).join("");if(!a.length)throw new Error;i.exerciseList.innerHTML=r,i.categoryContainer.innerHTML="",document.querySelector(".search-input").value="",$(t,n),E(t)}catch({message:a}){i.exerciseList.innerHTML="",m.error({title:"Error",message:`Failet to fetch exercises ${a}`})}},R=async({category:e,query:s,currentPage:a,searchTarget:n})=>{i.exerciseList.innerHTML=`<li>${B()}</li>`;try{const{results:t,totalPages:r,page:o}=await N(e,s,a,n);let c=t.map(({target:d,rating:v,burnedCalories:S,bodyPart:g,time:y,name:l,_id:x})=>{const F=String(v.toFixed(1));return`
    ...
    `}).join("");i.categoryContainer.innerHTML=`<span class="exercise-category">${b(f.query)}</span>`;let p;return c.trim()===""?(p=!0,c='<h3 class="unsucces-title">Unfortunately, <span>no results</span> were found. You may want to consider other search options to find the exercise you are looking for. Our range is wide and you have the opportunity to find more options that suit your needs.</h3>'):p=!1,i.exerciseList.innerHTML=c,p?$():$(o,r),E(o),p}catch(t){console.log(t),m.error({title:"Error",message:"Failed to fetch information about exercises"}),i.exerciseList.innerHTML="",i.exerciseSearchForm.style.display="none"}};let q=1;const M=e=>{const s=e.map(({name:a,bodyPart:n,burnedCalories:t,time:r,target:o,_id:c})=>`
             ...
             <div class="wrapp">
                ...
                <div class="info-wrapper">
                  ...
                  <ul class="info-list">
                    ...
                    <li class="info-item">Body part: <span>${n}</span></li>
                    <li class="info-item">Target: <span>${o}</span></li>
                  </ul>
            </li>
    `).join("");i.favList&&(i.favList.innerHTML=s,i.emptyWrapper.style.display=e.length===0?"flex":"none",E(q))};f.currentPage!=="index.html"&&M(P(q));i.pagination.addEventListener("click",e=>{if(e.target.classList.contains("pages_list-btn")){const s=parseFloat(e.target.textContent);if(isNaN(s))return;const a=document.querySelector(".exercise-info"),n=document.querySelector(".exercises-section"),t=document.querySelector(".fav-wrapper");n&&(n.scrollIntoView(),a?R({category:f.category,query:f.query,currentPage:s,searchTarget:f.searchTarget}):A(f.query,s)),t&&(t.scrollIntoView(),q=s,M(P(q)))}});const J=e=>{const a=JSON.parse(localStorage.getItem("Favorites")).filter(({_id:n})=>n!==e);localStorage.setItem("Favorites",JSON.stringify(a)),m.success({title:"Succes",message:"Succesfully deleted from your favorite list"})},_=e=>`
  ...
  `;i.backdrop.querySelector(".exercise").innerHTML=l,i.backdrop.classList.remove("hidden"),document.body.classList.add("noScroll")}catch{m.error({title:"Error",message:"Failed to fetch current exercise"}),setTimeout(()=>{i.backdrop.classList.add("hidden")},0)}};export{i as a,R as b,z as c,Q as d,$ as e,J as f,f as g,M as h,A as r,U as s};
//# sourceMappingURL=modal-D8Jq9qR8.js.map
