import{r,g as i,l as y,a as B,c as p,b as C,s as f,d as k}from"./assets/modal-DT07hYYd.js";import{i as x}from"./assets/vendor-CLTxG5yw.js";const g=(e=0,t=0,s=3)=>{const n=Math.floor(s/2);let a=e-n,l=e+n;a<1&&(a=1,l=s),l>t&&(l=t,a=t-s+1),a<1&&(a=1);const d=[];for(let c=a;c<=l;c++)d.push(c);const o=[];//! Left "..."
...
o.push(t);//! Last page
            ...
            </li>`).join("");r.pagination.innerHTML=u,[...document.querySelectorAll(".pages_list-btn")].forEach(c=>{c.textContent.trim()==="..."&&c.classList.add("disabled")})};r.pagination.addEventListener("click",e=>{if(e.target.classList.contains("pages_list-btn")){const t=parseFloat(e.target.textContent);document.querySelector(".exercise-info")?m({category:i.category,query:i.query,currentPage:t,searchTarget:i.searchTarget}):h(i.query,t)}});const L=e=>{[...document.querySelectorAll(".pages_list-btn")].forEach(s=>{const n=parseFloat(s.textContent),a=parseFloat(e);n===a?(s.classList.add("active"),s.disabled=!0):(s.classList.remove("active"),s.disabled=!1)})},h=async(e,t)=>{r.exerciseList.innerHTML=`<li>${y()}</li>`;try{const{results:s,totalPages:n,page:a}=await B(e,t),l=s.map(({imgUrl:d,name:o,filter:u})=>`
          <li class="exercise-item" style="--img: url(${d})">
            <h3 class="exercise-subtitle">${p(o)}</h3>
            <p class="exercise-name">${u}</p>
          </li>
          `).join("");r.exerciseList.innerHTML=l,r.categoryContainer.innerHTML="",r.exerciseSearchForm.classList.add("hidden"),document.querySelector(".search-input").value="",g(a,n),L(a)}catch({message:s}){x.error({title:"Error",message:`Failet to fetch exercises: ${s}`})}},m=async({category:e,query:t,currentPage:s,searchTarget:n})=>{r.exerciseList.innerHTML=`<li>${y()}</li>`;try{const{results:a,totalPages:l,page:d}=await C(e,t,s,n);let o=a.map(({target:c,rating:$,burnedCalories:q,bodyPart:E,time:F,name:S,_id:T})=>{const w=String($.toFixed(1));return`
